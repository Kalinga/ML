...
When the vehicle made an angle ${\theta }$ in a time period of ${\Delta t}$, the point P now moved by the same angle as ${\theta}$. Consider the vehicle speed \textit{v}, the displacement of the point \textit{P} on the image is given by

\begin{equation} \label{distance-in-pixel}
{d_1}= \frac{v \times \Delta t}{CF}
\end{equation} 

Where ${d_1}$ is the displacement in pixels, CF is the calibration factor of the camera that converts real world distance to distance in pixels, $\Delta t$ as given in \ref{delta-t}.
...
Combining the results of eq \ref{new-xy} and \ref{new-height-width-pixels}, bounding box for the pedestrian is calculated as $({x_1}, {y_1},{p_w}, {p_h})$
