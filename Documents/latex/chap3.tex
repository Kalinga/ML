...
Where ${d_1}$ is the displacement in image space, $\Delta t$ as given in \ref{delta-t}.
This equation gives the metric displacement (${d_1}$) amount in the image space and can be converted to number of pixels units using dimensions of effective image area and pixel size.
...
{\lambda} is the scale factor \\
K is the constant distance that is not captured within the vehicle's camera and usually known as \textit{blind spot}. 
