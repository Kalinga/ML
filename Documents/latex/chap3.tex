...
the sigmoid derivative is well below the 1.0 in the whole range of input values. And this makes the gradients vanish quite fast as multiplying this value \footnote{value less than 1} several times for several layers, \textit{ back propagation} brings the value close to 0.
...
%Where ${P}_{\hat{t}}$ is the position of the pedestrian at time stamp $(t+\Delta t)$, and $\Delta t$ is given by below equation.
Where ${P}_1$ is the position of the pedestrian at time stamp $\hat{t}$, as given by below equation.
...
$(x_1,y_1,w_1,h_1)$ in the equation depends on basically three factors speed of the vehicle, speed of the pedestrian and the angle between vehicle camera and pedestrian. Also it is necessary to be noted that other social aspect such as obstacle, neighborhood pedestrian, sudden change in pedestrian's own decision for the destination and displacement of the camera will have impact.
...
Consider ($P$) as the the top left point of the pedestrian bounding box as the initial position of the pedestrian (x,y) on the image frame-of-reference and (w,h) as bounding box width and height respectively.
...
d is distance between camera and object and f is the focal length of camera. Consider a fixed focal length for the camera in a particular setting $\lambda$ is a function of camera-object distance and shall be precomputed. % to be applied in the equation \ref{object-distance}
...
\pagebreak
Where ${\lambda}$ is the scale factor, ${d_1}$ is the displacement in image space, $\Delta t$ as given in \ref{delta-t}. This equation gives the metric displacement (${d_1}$) amount in the image space and can be converted to number of pixels units using dimensions of effective image area and pixel size.
...
Combining above results with eq \ref{new-xy}, the bounding box for the pedestrian is approximated as $({x_1}, {y_1},{w_1}, {h_1})$, which helps in LSTM state refinement.
...
After the model is trained, its accuracy and performance needs be measured and that is described in detail in the subsequent chapters.
...
Intersection Over Union (IOU) is a measure similar to Jaccard Index that measures the overlap ratio between two bounding boxes. It evaluates the ratio between area of overlapping between two BBs and area of union between them.
