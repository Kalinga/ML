...
The above image was captured when an object (a white board) was kept at distance of 6m and 339 degree angle with the object.
...
The above image was captured when an object was kept at distance of 6m and 14 degree angle with the object.
...
\end{align*}
\begin{center}
\caption{Illustration of vehicle's angular motion}
\end{center}
\end{figure}
...
Where ${d_{yaw}}$ is the displacement in the image measured in pixel due to camera rotation ${\theta}$ degrees, and $fw$ is the frame width measured with number of pixels, and $AoV$ is the angle of view.
...
A point to be noted that, there is a minute change in the y-displacement due to the YAW-motion of the camera as seen in the experiment and in the above equation it is neglected for the estimation.
...
$d_o$, $w_o$, $f$, $w$ are measured in SI units and $w$ can be converted to number of pixel using the image specification.
