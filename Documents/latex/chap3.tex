...
every node/.style = {draw=black, fill=gray!30, 
                     minimum width=2cm, minimum height=1.0cm,
                     align=center},
every path/.style = {draw, -latex}
                        ]
\node[rounded corners=.55cm] (start) {Start};
\node (0) [below=7mm of start]{Image data is prepared with data augmentation}; 
\node (1) [below=10mm of 0]{A set of default bounding boxes associated with each feature map cell}; 
\node (2) [below=10mm of 1]{ At each feature map cell, offsets relative to default box shapes in the cell is predicted}; 
\node (3) [below=10mm of 2]{ Per-class score that indicates class instance is predicted}; 
\node (4) [below=10mm of 3]{ Each ground truth box is matched with the default box with best jaccard overlap};
\node (5) [below=10mm of 4]{ Match default box to any ground truth with jaccard overlap greater than 0.5}; 
\node (6) [below=10mm of 5]{ A combined loss function and back propagation applied end-to-end}; 
\node (sevralepocs) [below=10mm of 6]{ Train it for several epochs}; 
\node (7) [below=10mm of sevralepocs]{ Validate and save the neural network model};
\node[draw, diamond, aspect=2, below=7mm of 7] (validation) { Is validation error accepted?};
\node[rounded corners=.55cm] (end) [below=7mm of validation]  {END};
...
\draw   (7) -- (validation);
\draw   (validation) -- (end);
%\draw [arrows,red] (validation.west) to["No" '] + (-1,0) |-  (sevralepocs.west);

%\path [line] (validation)  |- node [right=50mm of validation,auto,text width=4em] {No} (sevralepocs);
%\path [line] (validation)  -- node [left=50mm of validation,auto,text width=4em] {Yes} (sevralepocs);


\end{tikzpicture}
\begin{center}
\caption{SSD network training}
\end{center}
\end{center}
\end{figure}[H]